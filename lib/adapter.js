...
module.exports = (function() {
  ...
  var adapter = {
    identity: 'sails-postgresql-adikus',
    ...
    createEach: function(table, records, cb) {
      spawnConnection(function __CREATE_EACH__(client, cb) {
        ...
        async.each(records, function(data, cb) {
          ...
          // Transform the data object into arrays for parametrized query
          var attributes = utils.mapAttributes(data),
              columnNames = attributes.keys.join(', '),
              paramValues = attributes.params.join(', ');
    ...
    update: function(table, options, data, cb) {
      ...
      // Escape Table Name
      var tableEscaped = utils.escapeName(table);

      // Build Query
      var query = new Query(dbs[tableEscaped.replace(/["']/g, "")].schema).update(tableEscaped, options, data);

      if(dbs[table].config.batchSaves){
          return queues[table].push({query: query, callback: cb, id: data.id || options.where.id});
      }

      return spawnConnection(function __UPDATE__(client, cb) {
  ...
  function spawnConnection(logic, config, cb) {
    ...
    function after(err, client, done) {
      ...
      return logic(client, function(err, result) {
    ...
    setInterval(function() {
        _(queues).each(function(queue, table){
            if(queue.length > 0){
                var query = '';
                var _query = new Query(dbs[table].definition);
                var currentQueue = queue;
                _(currentQueue).chain().pluck('query').each(function(q){
                    var orig = q.query;
                    _(q.values).each(function(val, key){
                        var escaped = val && val.replace ? "'"+val.replace(/'/g, "''").replace(/\$/g, '%$$%')+"'" : val || 'null';
                        q.query = q.query.replace('$'+(key+1), escaped);
                    });
                ...
                spawnConnection(function __UPDATE__(client, cb) {
                    ...
                    var timer = new Date();
                    client.query(query, function(err, result) {
                        //console.log('Executed',currentQueue.length,'statements in',(new Date()).getTime() - timer.getTime());
                        if(err) return cb(err);
                ...
                }, dbs[table].config, function(err, result) {
                    if(err){
                        console.error(err);
                        _(currentQueue).each(function(elem) {
                            elem.callback(err, []);
                        });
                    }
                    else{
                        var callbacks = {};
                        _(currentQueue).each(function(elem) {
                            callbacks[elem.id.toString()] = elem.callback;
                        });
                        _(result).each(function(res) {
                            if(callbacks[res.id])callbacks[res.id](err, [res]);
                        });
                    }
                });
            }
